...
export default function ImageDetail() {
  ...
  useEffect(() => {
    ...
    setForms(allForms);
    setCurrentForm(allForms[0]);//デフォルトは通常形態をセット
    setDefaultFormId(allForms[0]?.id);
  ...
  return (
    <div className="min-h-screen flex justify-center p-4">
      <main className="flex flex-col w-full max-w-6xl">
        <h1 className="text-3xl font-bold p-4">ポケモン詳細</h1>
...
//基本(画像・タイプ等)の表示
function BaseData({ pokemon, forms, currentForm, setCurrentForm, defaultFormId }) {
  ...
  return (
    <div className="p-6 m-4 space-y-2 border border-gray-300 rounded-2xl flex flex-col justify-center items-center text-center shadow shadow-gray-400">
      {/* No. */}
      <p className="text-sm text-gray-600 mb-0">No.{defaultFormId}</p>
      {/* 名前 */}
